...
function motion_update(self::FastSlam1, speed, yaw_rate, time_interval)
  for p in self.particles
    motion_update(p, speed, yaw_rate, time_interval, self.motion_noise_rate_pdf)
  end
end

function set_max_likelihood_pose(self::FastSlam1)
...
function resampling(self::FastSlam1)
