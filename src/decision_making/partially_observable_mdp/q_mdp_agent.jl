...
function init_value(self::QmdpAgent)
...
function init_policy(self::QmdpAgent)
...
function reward_per_sec(self::QmdpAgent)
  return (-1.0 - self.puddle_depth*self.puddle_coef)
end

function to_index(self::QmdpAgent, pose)
...
function draw_decision!(self::QmdpAgent, observation)
  # reached goal
