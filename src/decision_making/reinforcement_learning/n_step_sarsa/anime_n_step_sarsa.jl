...
# learning method: n-step sarsa

module AnimeNstepSarsa
  using Plots
  pyplot()
  ...
  include(joinpath(split(@__FILE__, "src")[1], "src/decision_making/reinforcement_learning/n_step_sarsa/n_step_sarsa_agent.jl"))

  function main(delta_time=0.1, end_time=50000; is_test=false)
    # save path of gif file
    path = "src/decision_making/reinforcement_learning/n_step_sarsa/anime_n_step_sarsa.gif"
    ...
    ekf = ExtendedKalmanFilter(init_pose, env_map=map,
                               dist_dev_rate=0.14,
                               dir_dev=0.05)
    agent = NstepSarsaAgent(delta_time=delta_time, estimator=ekf)
