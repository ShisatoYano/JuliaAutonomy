...
mutable struct SarsaLambdaAgent
  speed
  yaw_rate
  delta_time
  estimator
  ...
  # init
  function SarsaLambdaAgent(;delta_time::Float64=0.1,
                            estimator=nothing,
                            puddle_coef=100,
                            alpha=0.5,
...
function set_action_value_function(self::SarsaLambdaAgent)
  state_space = Dict()
...
function init_policy(self::SarsaLambdaAgent)
...
function reward_per_sec(self::SarsaLambdaAgent)
  return (-1.0 - self.puddle_depth*self.puddle_coef)
end

function to_index(self::SarsaLambdaAgent, pose)
...
function policy(self::SarsaLambdaAgent, pose)
...
function q_update(self::SarsaLambdaAgent, r, s_, a_)
  if self.s === nothing
    return
  end
  ...
  # difference between previous action value and observed action value
  diff_q = r + q_ - q 

  # update
  for i in 1:length(self.s_trace)
    s_back, a_back = self.s_trace[end-(i-1)], self.a_trace[end-(i-1)]
    self.state_space[s_back].q[a_back] += self.alpha*diff_q*(self.lmd^(i-1)) 
  end
end

function draw_decision!(self::SarsaLambdaAgent, observation)
  # finished learning
  ...
  if self.estimator !== nothing
    ...
    # q-learning
    q_update(self, r, s_, a_) # update using reward and next state
    self.s, self.a = s_, a_
