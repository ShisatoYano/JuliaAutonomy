...
mutable struct SarsaAgent
  ...
  # init
  function SarsaAgent(;delta_time::Float64=0.1,
                      estimator=nothing,
                      puddle_coef=100,
                      alpha=0.5,
...
function set_action_value_function(self::SarsaAgent)
  state_space = Dict()
...
function init_policy(self::SarsaAgent)
...
function reward_per_sec(self::SarsaAgent)
  return (-1.0 - self.puddle_depth*self.puddle_coef)
end

function to_index(self::SarsaAgent, pose)
...
function policy(self::SarsaAgent, pose)
...
function q_update(self::SarsaAgent, r, s_, a_)
  if self.s === nothing 
  ...
  # update q
  self.state_space[self.s].q[self.a] = (1.0 - self.alpha)*q + self.alpha*(r + q_)
end

function draw_decision!(self::SarsaAgent, observation)
  # finished learning
  ...
  # time limit learning
  if self.learning_timer > 30.0 && self.in_goal == false
    self.update_end = true
  end
