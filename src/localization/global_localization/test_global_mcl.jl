...
module TestGlobalMcl
  ...
  function single_test(time_interval=0.1, test_time=30; particle_num=1000)
  ...
  function main(test_num=1000)
    ...
    for test in 1:test_num
      println("test: $(test)")
      result = single_test(particle_num=1000)
      if result == true
        convergence_count += 1
      end
    end

    println("Global Mcl Accuracy:$(convergence_count)/$(test_num)")
  end
end