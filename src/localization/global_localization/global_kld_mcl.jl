...
mutable struct GlobalKldMcl
  ...
  function GlobalKldMcl(max_num::Int64; motion_noise_stds::Dict=Dict("nn"=>0.20, "no"=>0.001, "on"=>0.11, "oo"=>0.20),
    ...
    self.particles = [Particle([0.0, 0.0, 0.0], 1.0/max_num) for i in 1:max_num]
...
function motion_update(self::GlobalKldMcl, speed, yaw_rate, time_interval)
  if self.observed == false && length(self.particles) == self.max_num
    for p in self.particles
...
function set_max_likelihood_pose(self::GlobalKldMcl)
  max_index = argmax([p.weight for p in self.particles])
  self.max_likelihood_particle = self.particles[max_index]
  self.estimated_pose = self.max_likelihood_particle.pose
end

function observation_update(self::GlobalKldMcl, observation)
  for p in self.particles
...
function draw!(self::GlobalKldMcl)
