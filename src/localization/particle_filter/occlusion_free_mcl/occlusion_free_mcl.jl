...
include(joinpath(split(@__FILE__, "src")[1], "src/localization/particle_filter/occlusion_free_mcl/occlusion_free_particle.jl"))
include(joinpath(split(@__FILE__, "src")[1], "src/model/map/map.jl"))

mutable struct OcclusionFreeMcl
  particles
  ...
  # init
  function OcclusionFreeMcl(init_pose::Array, num::Int64,
                            motion_noise_stds::Dict;
                            env_map=nothing,
                            dist_dev_rate=0.0,
                            dir_dev=0.0)
    self = new()
    self.particles = [OcclusionFreeParticle(init_pose, 1.0/num) for i in 1:num]
    self.map = env_map
...
function motion_update(self::OcclusionFreeMcl, speed, yaw_rate, time_interval)
  for p in self.particles
    motion_update(p, speed, yaw_rate, time_interval, self.motion_noise_rate_pdf)
  end
end

function set_max_likelihood_pose(self::OcclusionFreeMcl)
  max_index = argmax([p.weight for p in self.particles])
  self.max_likelihood_particle = self.particles[max_index]
  self.estimated_pose = self.max_likelihood_particle.pose
end

function observation_update(self::OcclusionFreeMcl, observation)
  for p in self.particles
...
function resampling(self::OcclusionFreeMcl)
...
function draw!(self::OcclusionFreeMcl)
